...
void Projectile::CheckForCollision()
{
	...
	if ((mPosX > WIDE_BOARD_WIDTH + mApp->mDDInterface->mWideScreenOffsetX || mPosX + mWidth - mApp->mDDInterface->mWideScreenOffsetX < 0.0f) && !aTargetZombie)
	{
		Die();
		return;
	}
...
void Projectile::Draw(Graphics* g)
{
	...
	if (aImage)
	{
		...
		else
		{
			...
			TodScaleRotateTransformMatrix(aTransform, aOffsetX + mBoard->mX + mApp->mDDInterface->mWideScreenOffsetX, aOffsetY + mBoard->mY, mRotation, aScaleX, aScaleY);
