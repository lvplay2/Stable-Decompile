...
void SexyApp::ReadFromRegistry()
{
	...
	mReferId = SexyStringToString(GetString("ReferId", StringToSexyString(mReferId)));
		...
		if (fp != NULL)
		{
			for (;;)
			{
				...
				if (aLen < 256)
				{
					char aProdName[256];
					aProdName[aLen] = '\0';
					fread(aProdName, aLen, sizeof(SexyChar), fp);
...
void SexyApp::HandleCmdLineParam(const std::string& theParamName, const std::string& theParamValue)
{
	if (theParamName == "-version")
	{
		...
		MessageBoxA(NULL, aVersionString.c_str(), "Version Info", MB_ICONINFORMATION | MB_OK);
#else
...
void SexyApp::GetSEHWebParams(DefinesMap* theDefinesMap)
{
	...
	theDefinesMap->insert(DefinesMap::value_type("buildnum", StrFormat("%d", mBuildNum).c_str()));
...
void SexyApp::InitPropertiesHook()
{
	...
	LoadProperties(_S("properties\\partner.xml"), false, checkSig);
	...
	std::string aNewTitle = SexyStringToString(GetString("Title", _S("")));
	if (aNewTitle.length() > 0)
	mTitle = StringToSexyString(aNewTitle + " " + mProductVersion);
