...
//TODO: Make the special version (DONE)
			...
			for (int y = 0; y < theDestRect.mHeight; y++)
			{
				...
				for (int x = 0; x < theDestRect.mWidth; x++)
				{
					...
					if (a != 0)
					{
						...
						*(aDestPixels++) = (aNewDestAlpha << 24) |
							((((dest & 0x0000FF) * oma) >> 8) + (((src & 0x0000FF) * cr) >> 8) & 0x0000FF) |
							((((dest & 0x00FF00) * oma) >> 8) + (((src & 0x00FF00) * cg) >> 8) & 0x00FF00) |
							((((dest & 0xFF0000) * oma) >> 8) + (((src & 0xFF0000) * cb) >> 8) & 0xFF0000);
					}
					else
						aDestPixels++;
				}
